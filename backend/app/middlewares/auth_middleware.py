...
def verify_admin_session(
    ...
    if not session_token:
        return throw_error_response("Admin session token required", 6001)

    admin_session = (
        db.query(AdminSessions)
        .join(AdminUsers, AdminUsers.userId == AdminSessions.userId)
        .filter(
            AdminSessions.session_token == session_token,
    ...
    if not admin_session:
        return throw_error_response("Invalid or expired admin session", 6002)

    # 🔐 decrypt check
    decrypt = decrypt_data(session_token)
    if not decrypt or decrypt.get("userId") != admin_session.userId:
        return throw_error_response("Admin session mismatch", 6003)

    # ✅ THIS IS THE FIX
    admin_user = admin_session.adminuser

    request.state.adminUserId = admin_user.id          # ✅ INTEGER ONLY
    request.state.adminUserName = admin_user.admin_name
