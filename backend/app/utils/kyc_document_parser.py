...
def extract_details(text: str) -> dict:
    prompt = f"""
You are a highly accurate OCR document parser.

STRICT RULES:
- Output STRICT JSON ONLY
- No markdown, comments, or explanations
- Do NOT hallucinate missing values
- If a field is not present in OCR text, leave it empty ("") or null (for numbers/arrays)
- Always return "name" in English (transliterate if OCR text is in another language)
- Always return "address" in English (transliterate if OCR text is in another language)
- Always return "doc_number" with NO spaces (normalize by removing all whitespace characters)

DOCUMENT TYPES:
- aadhaar_card
- pan_card
- resume
- address_proof (electricity_bill, gas_bill)
- qualification (marksheet, degree, diploma, certificate)
- other
